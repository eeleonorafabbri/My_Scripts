    ...
    for key, value in json_obj.items():
        ...
        if isinstance(value, dict):
            ...
            for i, item in enumerate(value):
                flattened.update(flatten_json({str(i): item}, f"{new_key}{sep}{key}", sep=sep))
        else:
    ...
    if cell_df.loc[row_idx, 'm__glif'] != 0:
        ...
        for file in file_in_cartella:
            if file.endswith(".json") and "config" in file:
                ...
                neur_id = get_neur_id(specimen_id_list, neuronal_model_id_list, specimen_id)
                glif_param_df['Column'] = glif_param_df['Column'].str.replace((str(neur_id) + '_'), '')
    # else:
...
def merge_glif_df(specimens):
    ...
    for specimen_id in specimens:
        peri_param_dict, all_act_param_dict, glif_param_dict = get_dict(specimen_id, cell_df)
...
def _merge_glif_df2(specimens):
    ...
    GLIF_df['specimen_id'] = specimens
    for specimen_id in specimens:
        peri_param_df, all_act_param_df, glif_param_df = get_json_parameters(specimen_id, cell_df)
...
glif_specimens = [ 486111903, 548459652, 464198958, 314900022, 502999078,
                   585947309, 519749342, 586071425, 476131588, 561517025]
